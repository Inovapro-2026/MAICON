 ...
 */
export async function listUserBusinesses(userId: string): Promise<BusinessSummary[]> {
...
export async function resolveBusinessContext(
  ...
  if (membership.business.status === 'SUSPENDED' || membership.business.status === 'CANCELLED') {
    return null;
  }
