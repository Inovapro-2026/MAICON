...
export async function POST(req: NextRequest) {
  ...
  if (!res.ok || !data?.success) {
    ...
    return NextResponse.json({ success: false, error: { code: 'INVALID_CREDENTIALS', message } }, { status: 401 });
  }

  const payload = data.data as { token: string; user: unknown; must_change_password: boolean; active_business?: { id: string; name: string; slug: string; role: string; status: string } | null };
