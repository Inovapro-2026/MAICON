...
import { isOptedOut, registerOptOut, resolveOrCreateLeadByPhone, resolveOrCreateLeadByEmail } from '../services/leads';
...
export async function processMessageReceived(job: { id?: string; data: MessageReceivedData }): Promise<void> {
  ...
  let lead: { id: string; phone: string | null; email: string | null; status: string } | null = null;
  if (!leadId) {
    if (channel === 'WHATSAPP') {
      ...
      if (job.data.remoteJid?.includes('@lid')) {
        const resolved = await getWhatsAppManager(businessId).resolvePhoneFromLid(job.data.remoteJid);
        if (resolved) {
          digits = resolved;
          logger.debug('LID resolvido para telefone', { lid: from, phone: resolved });
        }
      }
  ...
  // 3) Conversa
  const conversationId = await ensureConversation(leadId, businessId ?? fullLead.business_id);
  await touchConversation(conversationId, businessId);

  // 4) Atualiza status do funil
  await prisma.lead.update({ where: { id: leadId }, data: { status: 'RESPONDED' } });
  ...
  if (conversation.human_handled) {
    logger.info('Conversa em modo manual; IA não acionada', { conversation_id: conversationId });
    return;
  }
  ...
  if (detectOptOut(content)) {
    await registerOptOut(leadId, channel, `Palavra-chave detectada: "${content.slice(0, 80)}"`, businessId ?? fullLead.business_id);
  ...
  await getWorkerQueue(QUEUE_NAMES.AI_RESPONSE).add(
    ...
    { jobId: `ai-${businessId ?? 'default'}-${leadId}-${Date.now()}`, attempts: 3, backoff: { type: 'exponential', delay: 2000 } }
  );
}
